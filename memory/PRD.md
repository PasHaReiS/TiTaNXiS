...
## Recent Changes
- **Feb 21, 2026 (v44)**: Sticky header genişletildi — 5 sayfaya inject edildi. `Members`: action toolbar (Guild CSV, Güç OCR, Kale/Rank OCR, +New) `<Header>` children'a. `Events`: aktif sayaç + OCR + New action row + Liste/Takvim view toggle inject edildi. `Reports`: ReportTabs + PeriodBar `<Header>` children'a. `PointsAbout`: pa-sidebar-list (Puan Ekle / Puan Listesi tabs) inject. `PointCalcPage`: pc-kind-tabs (Pre Event Scoring / Other Event Calculations + Excel/İçe Aktar) inject. Doğrulama: 430×800 viewport, `layoutScrollContainer.scrollTop=900` → `slotTop:0, slotBottom` stabil kalıyor 5 sayfada (uyeler·etkinlikler·raporlar·puanlar-hakkinda·puan-hesaplama), `tabsInSlot=True` her sayfada.
- **Feb 21, 2026 (v43)**: Header layout redesign — single sticky wrapper (top bar + breadcrumb + title + tabs) via `.app-shell` flex-column + `Layout` scroll container + `Header` children slot.
- **Feb 21, 2026 (v42)**: **P0 Sticky Header FIX** (superseded by v43).
