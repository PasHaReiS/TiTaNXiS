...
## Recent Changes
- **Feb 18, 2026 (v22 fix)**: **OCR post-save freeze bug** — after "Onayla & Kaydet" the screen appeared to hang with only the dark backdrop showing content underneath the modal frozen. Root cause: OcrDialog kept its internal state (`previews`, `result`, `applying`, `selection`, `excludedRows`, `rowEdits`, `rawChunks`, `autoAppliedRows`, `allianceFilter`, `showNewList`) between opens. When the parent re-rendered with fresh SWR data, the stale state layer plus a `setApplying(false)` call landing after `onClose()` produced an intermediate "empty modal" flash. Fix: added a dedicated `useEffect([open])` that hard-resets every state on close, moved the `setApplying(false)` before `onClose()` so the applying flag never trails a null return, and added an early `return` after close so nothing else runs post-unmount. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
