...
- **[2026-02] Commander "Tümü" (KOMUTANLAR aggregate) flat view**: Aggregate section renders as a single flat 2-col grid of Tetikçi + Bombacı + Kalkanlı (category groups removed). `Robotlar` deliberately excluded upstream in `commanders` useMemo — robots only shown in their own sub-tab. Sort order still: KoF first → ranked descending → unranked by rarity → name tie-break. Other sections (Kafes/Garnizon/Savaş/Svs) keep grouped rendering.
