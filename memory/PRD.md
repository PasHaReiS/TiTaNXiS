...
## Recent Changes
- **Feb 18, 2026 (v14)**: (a) OCR **Bulk Rename Autolink** — inline `_lev`/`_fuzzyTopMatches` (Damerau-Levenshtein, tag-agnostic, relevance ≤ 0.45) surfaces up to 3 clickable `🔗 <name>` chips beneath any name that no longer matches an existing member. One tap replaces the row's edit with the roster hit. (b) **Second DeepL pass** run — recovered ~140 additional strings; the remaining short labels (~27/33 per locale) fall back through the new `fallbackLng: ["en", "tr"]` chain to English. (c) **Poll Export CSV** — admin-only `📥 TG Oy Dökümü İndir` chip in the details panel builds a UTF-8 CSV client-side from `results.tg_voter_details` (username, options, voted_at) with BOM for Excel. (d) **Guild Data CSV** — new backend `GET /api/reports/guild-data.csv` streams a per-member roll-up (name, alliance, rank, country, power, castle, total/active/archived points, event count) sorted by power; Members header exposes a `Guild CSV` button next to OCR.
