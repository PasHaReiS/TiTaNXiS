...
## Recent Changes
- **Feb 18, 2026 (v21)**: (a) **Legend Count Badges** — every alliance legend chip now trails a mono `N` pill (auto-tinted with the alliance colour when > 0, muted slate at 0). Counts respect manual edits + `[TAG]` fallback and exclude struck rows, so admins spot dominant clans at a glance; empty alliances fade to 45% opacity. (b) **Auto-Apply Preview** — rows the useEffect rewrote get a soft amber left-border (`#FCD34D`) + gradient inset until the admin overrides the input or hits Undo. Hover tooltip: "Otomatik bağlandı — düzenlersen bu vurgu kalkar". When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
