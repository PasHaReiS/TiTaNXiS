...
## Recent Changes
- **Feb 18, 2026 (v15)**: (a) **Fuzzy Suggest Confidence** — `🔗` Autolink chips paint a 4-tier confidence halo based on Levenshtein distance (`≤1` green glow "Yüksek eşleşme", `≤2` soft green "İyi", `≤3` amber "Yaklaşık", `≥4` muted orange "Zayıf"). Tooltip surfaces the raw distance. (b) **DeepL Retry Cron** — new `POST /api/cron/deepl-retry-i18n` scheduled nightly at 03:45 UTC via `/app/.emergent/crons.yml`. Auth via `WEBHOOK_CRON_SECRET`. Walks all non-{tr,en,ru,pt} locales, re-attempts missing v9 keys via `_deepl_translate_one` with 0.4s throttle, and patches `i18n/index.js` in place with a `// __V9_DEEPL_RETRY__` marker for anything that came back different from the TR source. Idempotent + placeholder-safe.
