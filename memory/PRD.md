...
- **[2026-02] Commander card rarity theming**: `RARITY` map extended with `bg` tone per tier. New `rarityCardStyle(rarity)` helper returns `{ border: 2px solid <tier-color>, background: linear-gradient(180deg, <bg>f5, <bg>cc) }`. Legendary → orange (#F97316 / #1a0d00), Epic → purple (#A855F7 / #1a0030), Common → blue (#3B82F6 / #001530). Applied to both `CommanderCard` and `CommanderGridCard`. Grid image frame + list-view image thumbnail borders also colour-matched. Cards without rarity fall back to the legacy `card-red-gold` red/gold frame.
