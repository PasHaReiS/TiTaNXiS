...
## Recent Changes
- **Feb 18, 2026 (v18)**: (a) **Alliance Colour Hint** — OcrDialog fetches `/alliance-colors` and paints an 8px glowing dot next to every `🔗` alliance suggestion chip AND next to the İttifak input when the entered name matches an existing alliance (case-insensitive lookup so GOW/GoW/GOw share their own dot). Admins recognise clans by colour before tapping. (b) **Autolink Auto-Apply** — new `useEffect` runs on preview arrival, walks each un-edited row, and auto-rewrites the name to the top existing member when Levenshtein ≤ 1 (green "Yüksek eşleşme"). Manual edits (`rowEdits[i].name !== undefined`) are always respected; admins can revert by typing back. Applies to both members & event modes. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
