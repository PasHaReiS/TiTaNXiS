...
## Recent Changes
- **Feb 18, 2026 (v25)**: **Smart Alliance Tag Detection** — new module-level helpers `_ALLIANCE_TAG_RE = /\[([A-Za-z0-9]{2,5})\]/`, `_extractAllianceTag`, and `_stripTagAndJunk` (drops the bracket + any leading digits/separators). The event preview now auto-shows the cleaned name in the İsim input and pre-fills the İttifak input from the bracket even when the name looks like `"12 [GOW] Ekko"`. The tag regex accepts the bracket anywhere in the raw string, not just at position 0. `doApply` also derives a fallback `alliance_name` for event rows from the OCR raw name when the admin hasn't set one. Manual edits still win — no auto-matching against existing members. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
