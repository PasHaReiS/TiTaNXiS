...
- **[2026-02] Stone & Fire theme + Runic nav**: `index.css` fully re-themed (fractal-noise stone body bg, Cinzel + MedievalSharp fonts, lava-glow cards, stone-slab list rows, carved-stone form inputs, metallic podium 1/2/3 gold-silver-bronze glows, `.heading-cinzel` utility, `.ember-glow` keyframe). `BottomNav.jsx` replaced lucide icons with 6 custom inline SVG rune components (RuneTrophy / RuneSwords / RuneTablet / RunePlus / RuneWarriors / RuneTiwaz). Two CSS syntax regressions fixed en route (nested `*/` at line 363; orphaned `.bottom-nav-btn` block at 258-274). All data, i18n, auth, CRUD preserved.
