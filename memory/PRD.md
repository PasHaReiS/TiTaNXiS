...
## Recent Changes
- **Feb 18, 2026 (v26)**: OCR event preview modernised. (a) New table columns: **İttifak · Üye Adı · Rütbe · Puan** with softer amber-tinted header labels, roomier padding (`py-1.5 px-2`), and per-cell rounded hover / focus rings (`hover:bg-white/5 → focus:ring-1 focus:ring-amber-400/60`). (b) `_RANK_RE = /\b(R[1-5])\b/i` extracts the rank from the raw OCR name; the new `<select>` cell shows the detected rank in amber or a placeholder "—" when unreadable so admins pick R1-R5 manually. `_stripTagAndJunk` now also drops the rank token so it doesn't linger in the İsim input. (c) `doApply` maps `patch.rank` into `next.rank` (whitelisted R1-R5) and falls back to the OCR-name-extracted rank when the admin left the cell empty; backend accepts the extra field via extra=allow. Row-level 🗑 delete + collapsible new-members panel unchanged. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
