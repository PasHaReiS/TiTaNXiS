...
- **[2026-02] Note position + color (Member)**: Backend `Member`/`MemberCreate`/`MemberUpdate` extended with `note_position` (`inline` | `bottom`) and `note_color` (hex). Form has toggle chips + 8-color palette (kırmızı/turuncu/sarı/yeşil/mavi/mor/beyaz/altın) shown only when bottom is selected. Card + Profile Dialog render conditionally: inline → next to name text-xs red bold; bottom → under F8 as bold italic in chosen color.
