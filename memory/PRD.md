...
## Recent Changes
- **Feb 19, 2026 (v27)**: OCR event mode gained full 5-field extraction & editing. (a) `/app/backend/routes/ocr.py`: `_PROMPTS["event"]` and `_PROMPTS["members"]` rewritten to tell GPT-4o Vision to (1) return `castle_level` = the number next to the small pink/magenta hexagon icon in front of the player name (strip any letter prefix like `F8`→8), (2) return `alliance_name` WITHOUT any `[` or `]` characters, (3) recognise R1-R5 rank badges, (4) normalise power suffixes `1.5B / 800M / 250K` → raw integer. Added `_normalise_power`, `_clean_alliance`, `_postprocess_rows` helpers that scrub the LLM output after parse (both `/ocr/parse` and `/ocr/parse-multi`). (b) `/ocr/apply-event-points` now also backfills matched members with `castle_level / rank / bireysel_guc / alliance_name` when OCR read new stats, and new auto-created members inherit those stats. Response includes `member_updates` counter. (c) `/app/frontend/src/components/OcrDialog.jsx`: event preview table gained a new pink **Kale** column between Rütbe and Puan with a numeric input bound to `rowEdits[i].castle_level`. `_mergeRows` + `remergeWithStrategy` now preserve `castle_level / rank / alliance_name / power` across multi-image merges (first-non-empty-wins). `doApply` maps `patch.castle_level` and strips `[]` from `alliance_name` in the payload as a safety net.
