...
## Recent Changes
- **Feb 18, 2026 (v16)**: OCR members preview now edits alliance names. OcrDialog fetches `/alliances` and renders each row's İttifak cell as an `<input list=…>` bound to `rowEdits[i].alliance_name` with a datalist of the existing alliance names. Colour cue on input text: green when the entry matches an existing alliance, amber when it's a brand-new name (tooltip: "⚠ Yeni ittifak — kaydedilirse yeni bir grup açılır"). `doApply` maps the edit into `next.alliance_name` before POSTing to `/ocr/apply-members` (endpoint already respects the field). Admin panel alliance rename lives on Members page (already shipped via `alliance-rename-btn-<name>` pencil).
