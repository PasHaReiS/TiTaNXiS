...
## Recent Changes
- **Feb 19, 2026 (v38)**: OCR önizleme tabloları — HEM üye HEM etkinlik — tamamen inline-style rewrite. Tailwind className kaldırıldı; kalan tüm tabla satırları `style={{...}}` kullanıyor. **RANK_COLORS** yeni palet: R1=#22c55e yeşil · R2=#3b82f6 mavi · R3=#a855f7 mor · R4=#f97316 turuncu · R5=#ef4444 kırmızı (dropdown seçim değişince renk anında güncelleniyor). **Header'lar**: ✓ 30px · 🗑 30px · İTTİFAK 80px · ÜYE ADI (esnek) · RANK/RÜTBE 56px + (üye modu) KALE 58px · GÜÇ 80px / (etkinlik modu) PUAN 80px — hepsi `#fbbf24` amber `fontSize 10px bold uppercase letterSpacing 0.05em`. **✓ sütunu** artık gerçek `<input type="checkbox" checked={!isExcluded}>` (yeşil `accentColor`). **🗑 sütunu** `<button>` sade — elenmiş satır için ↺, aktif için 🗑, kırmızı `#ef4444`. **İttifak** pill badge: `borderRadius: 9999px · background rgba(251,191,36,0.15) · color #fbbf24 · border rgba(251,191,36,0.5) · textAlign center · fontWeight bold`. **KALE** dropdown `[1..10]` OCR okunmuşsa otomatik seçili, `#F472B6` pembe. **GÜÇ** `#f97316` turuncu sağ hizalı monospace. **PUAN** aynı `#f97316` sağ hizalı. Input arka planı `rgba(0,0,0,0.3)` border `rgba(255,255,255,0.1)` borderRadius 4px. Tablo `width:100%` auto-layout; ekrana sığar.
