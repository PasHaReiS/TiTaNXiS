...
## Recent Changes
- **Feb 18, 2026 (v12b)**: Attempted DeepL bulk-translate backfill for 23 remaining locales via `/tmp/deepl_backfill.py` (curl-based to bypass local urllib WAF blocks). Blocked by DeepL rate-limit / quota (HTTP 429 from api-free.deepl.com). Script is idempotent — re-run once quota resets to auto-patch `i18n/index.js` in place.
