...
## Recent Changes
- **Feb 18, 2026 (v19)**: (a) **Auto-Apply Toast** — the Autolink useEffect now fires a `✨ N isim otomatik bağlandı` sonner toast (2.6s) whenever it rewrites rows so admins have a visible receipt of what the fuzzy matcher touched. Zero-patch runs stay silent. (b) **Alliance Colour Legend** — compact swatch strip renders directly above the OCR preview table in members mode: each existing alliance gets an 8px glowing dot + name, styled with a dashed neutral outline. Admins learn every clan's colour before scanning rows. Both features rely on the already-fetched `/alliances` + `/alliance-colors` — no extra network calls. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
