...
- **[2026-02] Stone & Fire theme (comprehensive)**: `index.css` repainted with volcanic palette — base `#0D0B0A` body + cracked-stone SVG overlay, lava orange `#E74C1A` primary, fire red `#C0392B`, amber gold `#D4730A`, cream text `#F5F0E8`. Added `:root` CSS custom properties (`--bg-primary`, `--accent-primary`, etc.) + global Tailwind class overrides (`.bg-red-*`, `.bg-yellow-*`, `.bg-gray-*`, `.text-white`, `.border-*-*`) via `!important` so all utilities inherit fire palette. Cards, divider-glow, bottom-nav, buttons, chips, tree-items, scrollbar all use lava/amber tones with inset glow. Rank badges, rarity frames, i18n, responsive, data — all preserved.
