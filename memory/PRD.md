...
- **[2026-02] Commander form: hide Rank + Rarity for Team & Garrison**: `CommanderForm` in `Commanders.jsx` now computes `hideRankAndRarity = isTeamMode || isGarrison`. Rank chips/input + Rarity picker block is skipped when the current section is `KAFES ETKİNLİK`, `SAVAŞ`, `SVS EKİP`, or `GARNİZON`. Submit body sends `rank: null, rarity: null` for those forms. Tetikçi/Bombacı/Kalkanlı/Robotlar (KOMUTANLAR section) still show both fields.
