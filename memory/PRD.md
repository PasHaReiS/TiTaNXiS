...
## Recent Changes
- **Feb 21, 2026 (v42)**: **P0 Sticky Header FIX**. `html, body { overflow-x: hidden }` (`/app/frontend/src/index.css` L6-9) sticky descendant'ları (Breadcrumb + Header) viewport'a pin'lenmesini engelliyordu — Chrome/Safari'de bilinen bir stacking context bug'ı. Fix: `overflow-x: hidden` kuralı hem `html/body`'den hem de `.app-shell`'den kaldırıldı (`position: relative` de .app-shell'den çıkarıldı — sticky ancestor olma riski). Verify: 430×800 viewport'ta 800px scroll sonrası Breadcrumb `top=0`, Header `top=32` (data-testid="breadcrumb" + `<header>` `getBoundingClientRect().top` ile). Yatay overflow kontrolü: `documentElement.scrollWidth === window.innerWidth` (1920/1920) — kayma yok. Uyarı yorumu CSS'e eklendi: gelecekte hiçbir ancestor'a overflow eklenmemesi için.
