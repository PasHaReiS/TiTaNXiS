...
## Recent Changes
- **Feb 18, 2026 (v20)**: (a) **Auto-Apply Undo** — the Autolink useEffect now remembers each rewritten row in `autoAppliedRows` and the preview header exposes an amber "↩ N otomatik bağlamayı geri al" chip that strips just the `name` overrides for the whole auto-batch (manual edits + points/alliance overrides are preserved). Chip disappears once the batch is undone. (b) **Legend Filter** — each alliance legend chip is now a toggle; picking one filters the preview `<tbody>` to only that alliance (case-insensitive against `rowEdits[i].alliance_name` → OCR field → bracket tag). Active chip glows with the alliance colour, a "× filtreyi temizle" link clears it in one tap. Both features are members-mode only. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
