...
- **Feb 9, 2026 (v142.13 — CRITICAL: Bireysel Güç OCR "Tümünü Ekle" Hiçbir Şey Kaydetmiyordu)** — Backend:
  ...
  - **onSelect signature değişti**: `(pickedName, meta)` → `meta.isNew` bool + `meta.member` full row.
  - **i18n**: 3 yeni anahtar (tr + en) — `match_sheet_new_member_title/subtitle`, `ocr_match_new_toast`.
  - Frontend derleme temiz, deployment_agent PASS ✅
...
- **Feb 7, 2026 (v141 — Refactor Phase 3 + i18n Lazy Load + Announcement Protocol + PC Excel Wizard)** — Backend + Frontend:
  ...
  - **i18n Lazy Load**: `index.js` yeniden yazıldı. Bootstrap **sadece TR** statik import; diğer 28 dil `import('./langs/${code}.js')` (Webpack template-literal code-split) ile `changeLanguage()` çağrısında JIT yüklenir. `preloadLanguage(code)` yardımcı ihraç edildi (hover-preload için). **Kritik fix**: iter57 testinde `parseMissingKeyHandler: (key) => key` opsiyonunun `t(key, {defaultValue})` çağrılarını kırdığı bulundu → kaldırıldı. Initial bundle ~29× küçüldü.
  - **PC Excel İş Akışı Wizard**: `components/PCExcelWizard.jsx` yeni. 3-adım modal: (1) İndir — `GET /point-calc/export`, (2) Çevrim-dışı düzenle — sekme bilgilendirme, (3) Yükle — `POST /point-calc/import` + opsiyonel "Çevir" düğmesi (`POST /point-calc/translate-all`). Progress stepper, **drag-drop zone (`onDragOver`+`onDrop`)**, adım-adım renk geçişleri (turuncu → yeşil), Cinzel serif başlıklar, gradient premium UI. `data-testid`'ler: `pc-wizard-open-pre`, `pc-excel-wizard`, `pc-wizard-download`, `pc-wizard-file`, `pc-wizard-import`, `pc-wizard-translate`. `tr.js`'e 27 yeni `pc_wizard_*` anahtar eklendi (Sihirbaz, İndir, Düzenle, Yükle, adım açıklamaları). `PointCalcPage.jsx`'e Sparkles ikonlu "Sihirbaz" butonu eklendi (admin-only).
  - **Regression testing**: `testing_agent_v3_fork` → 26/26 pytest PASS (backend). 1 HIGH frontend bug (i18n defaultValue kırılması) tespit edildi ve düzeltildi + drag-drop eklendi + `if points:` guard eklendi. Tüm Phase 1+2+3 endpoint'leri (commanders, alliance meta, multiplier history, unit costs, point calc CRUD+export+translate-all, legal, reports, guild-target, members history/changes/bulk-*) canlı DB'de 200 döndü.
