...
## Recent Changes
- **Feb 18, 2026 (v23)**: OCR event mode simplified per admin request — table columns collapsed to **✓ · İttifak · İsim · Puan** (removed Kaynak, Durum, fuzzy chip suggestions, and the "N yeni üye oluşacak" panel). Alliance field is a full inline `<input list=…>` with the shared datalist. **Autolink Auto-Apply useEffect is now short-circuited** (early return); the 🔗 suggestions stay available in members mode but nothing auto-rewrites anymore — admins pick every match by hand. Fires immediate `setApplying(false)` before `onClose()` (v22 freeze fix) still in place. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
