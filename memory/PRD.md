...
## Recent Changes
- **Feb 21, 2026 (v46)**: Portal migration + Katılım Merkezi + Kristal spacing (DEPLOY-READY). Header renders via `ReactDOM.createPortal` into `#titanxis-header-slot` (a `flexShrink:0` direct child of `.app-shell` mounted by Layout). No `position:sticky` — header is a real flex child so mobile Safari sticky/transform bugs eliminated. `.app-shell` uses `100dvh` (dynamic viewport, handles mobile URL bar collapse). Reports title `Raporlar Merkezi` → `Katılım Merkezi`. Leaderboard: header-sticky-children `pb-2` → `pb-1`, `<div className="px-4">` `pt-1` added, group-strip `mb-4` → `mb-2` — Aktif/Arşiv ↔ Kristal chip gap trimmed to 9px verified. **Verification**: 430×800 viewport, Portal `slotFound:True stickyInsideSlot:True`, Leaderboard scroll 900px → header `top:0` stable, Members scroll 1500px → header stable, Events scroll → stable, Reports title = "Katılım Merkezi".
- **Feb 21, 2026 (v45)**: Sticky → Portal migration attempt (superseded by v46 which fixed extra `</div>` compile error).
- **Feb 21, 2026 (v44)**: Sticky header 5 sayfada genişletildi (Members, Events, Reports, PointsAbout, PointCalcPage).
