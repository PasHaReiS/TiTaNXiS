...
## Recent Changes
- **Feb 18, 2026 (v13)**: OCR safety layers — inline-edit typo detection + collapsible "N yeni üye oluşacak" panel with per-row + bulk elimination. DeepL bulk-translate backfill added ~759 v9 keys across 23 locales; the free-tier throttling swallowed some very short strings, so `/tmp/deepl_cleanup.py` pruned every key whose translated value still equalled the TR source. i18next now falls back to EN → TR only for those specific short labels while the long strings (Ordner, フォルダ, مجلد, etc.) stay natively translated. Both scripts (`/tmp/deepl_backfill.py`, `/tmp/deepl_cleanup.py`) are idempotent and can be re-run any time.
