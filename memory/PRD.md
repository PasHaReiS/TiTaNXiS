...
## Recent Changes
- **Feb 18, 2026 (v24)**: Leaderboard archive event modal — the per-member row now supports inline name edit. Extracted the row into `ArchiveMemberRow` component: clicking the name (admin only) turns it into an amber-underlined input, Enter/blur commits via `PATCH /members/{id}`, Escape aborts, and SWR invalidations refresh members + leaderboard + points caches. `#position` and total-points cells stay as buttons that still open the member profile. When the İttifak input holds a value that isn't in the existing `/alliances` list, up to two `🔗 <existing>` chips render below it with the same 4-tier Levenshtein confidence halo (`≤1` green glow → `≥4` muted orange). One tap remaps the row to that existing alliance; the free-text input still saves as a brand-new alliance if that's what the admin actually wants — no blocking, no merge.
