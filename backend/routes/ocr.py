...
_PROMPTS = {
    "members": (
        ...
        "\n\nCRITICAL — NUMBERS MUST BE EXACT, NOT COMPACTED:\n"
        "  • `power` is the FULL integer as displayed on screen. If the "
        "    screenshot shows '5.503.115' (or '5,503,115' / '5 503 115'), "
        "    return 5503115 — NEVER round to 5500000 or write '5.5M'.\n"
        "  • Do NOT interpret K/M/B suffixes as approximations. If a value "
        "    is genuinely displayed as '5.5M' (no full digits shown), then "
        "    return 5500000 verbatim; but if the FULL digit-grouped number "
        "    is visible, use that exact number character-for-character.\n"
        "  • Digit-group separators (`.`, `,`, or space) must be stripped; "
        "    return the raw base-10 integer only.\n"
        "  • Do NOT invent, average, or 'clean up' numbers. If a digit is "
        "    unreadable, return null for `power` — never guess.\n"
        "Rules: "
    ...
    "event": (
        ...
        "• `power` = individual might/combat power as a raw integer when visible on the row. "
        "  Normalise suffixes: B → *1e9, M → *1e6, K → *1e3. If not visible, null. "
        ...
        "IMPORTANT: Do NOT extract castle_level or rank for event scoreboards — these "
        "fields belong to the member-roster mode only. Return raw JSON only — no markdown "
