...
# ============================================================
# Simple folder taxonomy for archived events. Folders are just labelled
# containers — an event lives in at most one folder via `event.folder_id`.
# The Events archive tab and the Leaderboard archive picker both read from
# `/api/event-folders` so the two views stay in sync automatically.
...
class FolderAssignBody(BaseModel):
    event_ids: List[str]
    folder_id: Optional[str] = None  # None → move back to top-level


@api_router.get("/event-folders")
async def list_event_folders():
    docs = await db.event_folders.find({}, {"_id": 0}).sort("order", 1).to_list(500)
    # Attach a live count of archived events assigned to each folder so the
    # UI can render "Kupa 4 (12)" without a second round-trip.
...
async def delete_event_folder(folder_id: str, _: dict = Depends(require_edit)):
    # Deleting a folder does NOT delete its events — they just fall back to
...
async def assign_events_to_folder(body: FolderAssignBody, _: dict = Depends(require_edit)):
    ...
    if body.folder_id is not None:
        # Validate the target folder exists so a typo doesn't create an orphan reference.
...
async def reorder_event_folders(body: FolderReorderBody, _: dict = Depends(require_edit)):
    ...
    payload from the drag-and-drop reorder UI. Missing ids are silently
    skipped so a stale client doesn't hard-fail the batch."""
    ids = [i for i in (body.ids or []) if i]
    if not ids:
        return {"modified": 0}
    modified = 0
